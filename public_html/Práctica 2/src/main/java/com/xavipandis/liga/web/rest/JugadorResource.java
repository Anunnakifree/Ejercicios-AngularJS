...
public class JugadorResource {
    ...
    //jugadores canastas Ex2
    @RequestMapping(value = "/topAssist/{asistencias}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>> topAssist(@PathVariable Integer asistencias, Pageable pageable)
        throws URISyntaxException {
        Page<Jugador> page = jugadorRepository.topAssist(asistencias, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/jugadors/topAssist");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
